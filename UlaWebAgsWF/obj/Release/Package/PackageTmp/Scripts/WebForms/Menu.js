...
function Menu_Expand(item, VerticalOffset, verticalOffset, hideScrollers) {
    Menu_ClearInterval();
    ...
    if (child) {
        ...
        child.rel = tr.id;
        child.x = VerticalOffset;
        child.y = verticalOffset;
        if (Vertical) child.pos = "bottom";
...
function Menu_HoverDisabled(item) {
    ...
    Menu_Expand(node, data.VerticalOffset, data.verticalOffset); 
}
...
function Menu_Key(item) {
    ...
    var data = Menu_GetData(item);
    if (!data) return;
    var Vertical = Menu_IsVertical(item);
    var a = WebForm_GetElementByTagName(item, "A");
    var nextItem, parentItem, previousItem;
    if ((!Vertical && key == 38) || (Vertical && key == 37)) {
        ...
        if (previousItem) {
            ...
            Menu_Expand(previousItem, data.VerticalOffset, data.verticalOffset, true);
            event.cancelBubble = true;
            if (event.stopPropagation) event.stopPropagation();
            return;
        }
    }
    if ((!Vertical && key == 40) || (Vertical && key == 39)) {
        if (Vertical) {
            var subMenu = Menu_FindSubMenu(a);
            if (subMenu && subMenu.style && subMenu.style.visibility && 
                subMenu.style.visibility.toLowerCase() == "hidden") {
                Menu_Expand(a, data.VerticalOffset, data.verticalOffset, true);
                event.cancelBubble = true;
                if (event.stopPropagation) event.stopPropagation();
                return;
            }
        ...
        if (nextItem) {
            Menu_Focus(nextItem);
            Menu_Expand(nextItem, data.VerticalOffset, data.verticalOffset, true);
            event.cancelBubble = true;
            if (event.stopPropagation) event.stopPropagation();
            return;
        }
    }
    if ((!Vertical && key == 39) || (Vertical && key == 40)) {
        var children = Menu_Expand(a, data.VerticalOffset, data.verticalOffset, true);
        if (children) {
            var firstChild;
            ...
            if (firstChild) {
                ...
                Menu_Expand(firstChild, data.VerticalOffset, data.verticalOffset, true);
                event.cancelBubble = true;
                if (event.stopPropagation) event.stopPropagation();
                return;
            }
            ...
            while (parentItem && !Menu_IsVertical(parentItem)) {
    ...
    if ((!Vertical && key == 37) || (Vertical && key == 38)) {
        parentItem = Menu_FindParentItem(item);
        if (parentItem) {
            if (Menu_IsVertical(parentItem)) {
